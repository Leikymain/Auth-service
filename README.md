...
1. Crear un archivo `.env` con las variables requeridas (`mongodb_uri`, `secret_key`, credenciales SMTP, etc.).
2. Instalar dependencias con `pip install -r requirements.txt`.
3. Ejecutar el servicio con `uvicorn app.main:app --reload`.

La API quedará disponible en `http://localhost:8000`, con la documentación interactiva en `/docs`.
